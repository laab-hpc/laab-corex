#ifndef LAAB_CUBLASMP_UTILS_H
#define LAAB_CUBLASMP_UTILS_H
...
static inline void choose_process_grid(int nprocs, int *nprow, int *npcol)
{
...
static inline void *load_block_cyclic_matrix(const char *filename,
                                             ...
                                             int local_rows, int local_cols)
{
    ...
    for (int m = 0, local_start_col = 0; local_start_col < local_cols; ++m, local_start_col += block_cols) {
        ...
        for (int k = 0, local_start_row = 0; local_start_row < local_rows; ++k, local_start_row += block_rows) {
            ...
            if (local_start_row + tile_rows > local_rows || local_start_col + tile_cols > local_cols) {
                free(rowbuf);
                fclose(fp);
                free(local);
                return NULL;
