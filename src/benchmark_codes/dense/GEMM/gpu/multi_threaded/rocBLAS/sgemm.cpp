...
int main(int argc, char *argv[])
{
    ...
    fprintf(trace_file, "[LAAB-INFO] rocblas/sgemm | op_sizes=(m=%d, n=%d, k=%d) | flops=%.0f\n",
            m, n, k, flops);
    fflush(trace_file);
    ...
    float *d_A, *d_B, *d_C;
    hipMalloc((void **)&d_A, (size_t)m * (size_t)k * sizeof(float));
    hipMalloc((void **)&d_B, (size_t)k * (size_t)n * sizeof(float));
    hipMalloc((void **)&d_C, (size_t)m * (size_t)n * sizeof(float));

    hipMemcpy(d_A, A, (size_t)m * (size_t)k * sizeof(float), hipMemcpyHostToDevice);
    hipMemcpy(d_B, B, (size_t)k * (size_t)n * sizeof(float), hipMemcpyHostToDevice);
    ...
    for (int it = 0; it < reps; it++) {
        ...
        hipMemset(d_C, 0, (size_t)m * (size_t)n * sizeof(float));
        ...
        fprintf(trace_file, "[LAAB] rocblas/sgemm | rep=%d | dt=%s | dur=%.3f s | perf=%.2f GFLOP/s | GPU=%s | BUS=%s\n",
