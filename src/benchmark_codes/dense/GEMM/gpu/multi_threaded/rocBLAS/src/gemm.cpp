...
#define ROCBLAS_CHECK(expr) do { \
    rocblas_status err__ = (expr); \
    if (err__ != rocblas_status_success) { \
        fprintf(stderr, "rocBLAS error at %s:%d: status=%d\n", __FILE__, __LINE__, (int)err__); \
        return EXIT_FAILURE; \
    } \
} while (0)
...
static double matrix_l2(const void *buf, size_t count, const char *dtype)
{
    ...
    if (strcmp(dtype, "complex128") == 0) {
        const rocblas_double_complex *ptr = (const rocblas_double_complex *)buf;
        for (i = 0; i < count; ++i) {
            double real = ptr[i].real();
            double imag = ptr[i].imag();
...
static int gemm_once(rocblas_handle handle,
                     hipStream_t stream,
                     int m,
                     int n,
                     int k,
                     ...
                     double *elapsed_seconds)
{
    ...
    HIP_CHECK(hipMemsetAsync(d_C, 0, bytes_c, stream));
    ...
    if (strcmp(dtype, "float32") == 0) {
        ...
        const rocblas_double_complex alpha = {1.0, 0.0};
        const rocblas_double_complex beta = {0.0, 0.0};
        ROCBLAS_CHECK(rocblas_zgemm(handle,
                                    rocblas_operation_none, rocblas_operation_none,
                                    n, m, k,
                                    &alpha,
                                    (const rocblas_double_complex *)d_B, n,
                                    (const rocblas_double_complex *)d_A, k,
                                    &beta,
                                    (rocblas_double_complex *)d_C, n));
    } else {
...
int main(int argc, char **argv)
{
    ...
    HIP_CHECK(hipMemcpyAsync(d_A, A, bytesA, hipMemcpyHostToDevice, stream));
    HIP_CHECK(hipMemcpyAsync(d_B, B, bytesB, hipMemcpyHostToDevice, stream));
    ...
    HIP_CHECK(hipMemcpy(C, d_C, bytesC, hipMemcpyDeviceToHost));
    ...
    cpu = sched_getcpu();
    visible_dev = getenv("HIP_VISIBLE_DEVICES");
    if (!visible_dev) visible_dev = getenv("ROCR_VISIBLE_DEVICES");
    snprintf(pci_bus_id, sizeof(pci_bus_id), "%04x:%02x:%02x.0",
             prop.pciDomainID, prop.pciBusID, prop.pciDeviceID);

    fprintf(trace_file,
            "[LAAB-STEP] rocblas/%s | prob_size=A=%dx%d+B=%dx%d | prec=\"%s\" | flops=%.0f | interface=rocblas\n",
            kernel, m, k, k, n, dtype_a, flops);
    fflush(trace_file);

    fprintf(trace_file,
            "[LAAB-RUN] rocblas/%s | ts=%s | nthreads=%d | execution=HIP | l2_norm=%.12e\n",
            kernel, datetime, np, c_l2);
    fflush(trace_file);

    fprintf(trace_file,
            "[LAAB-HOST] rocblas/%s | hostname=%s | core_id=%d | gpu_id=%d | gpu_bus=%s | visible_devices=%s\n",
            kernel, hostname, cpu, device_id, pci_bus_id, visible_dev ? visible_dev : "not set");
    fflush(trace_file);
    ...
    for (int r = 0; r < reps; ++r) {
        ...
        fprintf(trace_file,
                "[LAAB] rocblas/%s | rep=%d | ts=%s | exec_time (s)= %.5f | perf (GFLOP/s)= %.5f\n",
